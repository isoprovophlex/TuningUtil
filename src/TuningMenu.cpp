...
namespace MPL::TuningMenu
{
    namespace
    {
        ...
        struct MenuEffectOverride
        {
            ...
            bool lumaVisitedThisSession = false;
            bool active = false;
            bool blurContributionRemoved = false;
            std::uint16_t blurCountBeforeOverride = 0;
            std::uint16_t expectedBlurCountAfterClose = 0;
            std::atomic<std::uint64_t> closeCleanupGeneration{ 0 };
        };
        ...
        enum class DynamicAmbientDragTarget
        {
            darkLimit,
            range,
            brightLimit,
        ...
        struct DynamicAmbientDragState
        {
            DynamicAmbientDragTarget target = DynamicAmbientDragTarget::darkLimit;
            float startMouseX = 0.0f;
            float startDarkLimit = 0.0f;
            float startBrightLimit = 255.0f;
        };
        ...
        std::unordered_map<ImGuiMCP::ImGuiID, DynamicAmbientDragState> dynamicAmbientDragStates;
        ...
        std::string SliderCreatorErrorText(const std::string_view a_error)
        {
            static constexpr std::array errors{
                ...
                std::pair{ "Slider-specific hue scales are supported only by filtered saturation sliders.", "sliderCreatorHueScalesRequireSaturation" },
                std::pair{ "Local links and slider-specific hue scales require a filtered weather slider.", "sliderCreatorLocalFeaturesRequireFilter" },
                std::pair{ "Every slider-specific hue scale must be a finite number.", "sliderCreatorInvalidHueScale" },
        ...
        void DisableFrameworkEffectsForLuma()
        {
            ...
            menuEffectOverride.lumaVisitedThisSession = true;
            ...
            if (menuEffectOverride.backgroundBlurEnabled)
            {
                ...
                {
                    menuEffectOverride.blurCountBeforeOverride = blur->blurCount;
                    menuEffectOverride.expectedBlurCountAfterClose = blur->blurCount > 0 ? blur->blurCount - 1 : 0;
                    if (blur->blurCount > 0)
                    {
        ...
        void RestoreFrameworkEffects(const bool a_menuRemainsOpen)
        {
            if (!menuEffectOverride.active)
            {
                return;
            ...
            if (menuEffectOverride.freezeTimeEnabled)
            {
                if (auto* main = RE::Main::GetSingleton())
                {
                    main->GetRuntimeData().freezeTime = a_menuRemainsOpen;
                }
            }
            ...
            if (menuEffectOverride.blurContributionRemoved)
            {
                if (auto* blur = RE::UIBlurManager::GetSingleton())
                {
                    if (a_menuRemainsOpen)
                    {
                        blur->IncrementBlurCount();
                    }
                    else
                    {
                        const auto targetCount = static_cast<std::uint16_t>(menuEffectOverride.blurCountBeforeOverride - 1);
                        while (blur->blurCount < targetCount)
                        {
                            blur->IncrementBlurCount();
                        }
                    }
            ...
            menuEffectOverride.blurCountBeforeOverride = 0;
        ...
        void ReleaseFrameworkEffectsAfterClose()
        {
            ...
            // SKSE Menu Framework 3.13 dispatches its close event after releasing its own freeze and blur effects.
            // Restoring the contribution here would reintroduce both effects after the panel is already closed.
            menuEffectOverride.active = false;
            menuEffectOverride.blurContributionRemoved = false;
            menuEffectOverride.blurCountBeforeOverride = 0;
        }

        void ReconcileClosedMenuState(
            const std::uint64_t a_generation,
            const bool a_reconcileFreeze,
            const bool a_reconcileBlur,
            const std::uint16_t a_expectedBlurCount)
        {
            if (menuEffectOverride.closeCleanupGeneration.load() != a_generation)
            {
                return;
            }
            if (const auto* mainWindow = SKSEMenuFramework::GetMainWindow(); mainWindow && mainWindow->IsOpen.load())
            {
                return;
            }

            // Menu Framework can deliver a final render after its close event. Keep the runtime
            // weather guard off during the same delayed cleanup window used for freeze and blur.
            ...
            const auto gameIsPaused = ui && ui->GameIsPaused();
            if (a_reconcileFreeze && !gameIsPaused)
            {
                if (auto* main = RE::Main::GetSingleton())
                {
                    auto& freezeTime = main->GetRuntimeData().freezeTime;
                    if (freezeTime)
                    {
                        freezeTime = false;
                    }
            ...
            if (a_reconcileBlur && !gameIsPaused)
            {
                if (auto* blur = RE::UIBlurManager::GetSingleton(); blur && blur->blurCount > a_expectedBlurCount)
                {
                    while (blur->blurCount > a_expectedBlurCount)
                    {
                        const auto countBeforeDecrement = blur->blurCount;
                        blur->DecrementBlurCount();
                        if (blur->blurCount >= countBeforeDecrement)
                        {
                            break;
                        }
                    }
        ...
        void ScheduleClosedMenuStateReconciliation(
            const bool a_reconcileFreeze,
            const bool a_reconcileBlur,
            const std::uint16_t a_expectedBlurCount)
        {
            const auto generation = ++menuEffectOverride.closeCleanupGeneration;
            std::thread(
                [generation, a_reconcileFreeze, a_reconcileBlur, a_expectedBlurCount]()
                {
                    for (std::uint8_t check = 0; check < 10; ++check)
                    {
                        std::this_thread::sleep_for(100ms);
                        if (menuEffectOverride.closeCleanupGeneration.load() != generation)
                        {
                            return;
                        }
        ...
        void __stdcall HandleMenuFrameworkEvent(const SKSEMenuFramework::Model::EventType a_event)
        {
            ...
            case SKSEMenuFramework::Model::kOpenMenu:
                ++menuEffectOverride.closeCleanupGeneration;
                menuFrameworkOpen = true;
                menuEffectOverride.lumaVisitedThisSession = false;
                menuEffectOverride.expectedBlurCountAfterClose = 0;
                if (WeatherLock::IsEnabled())
                {
                    WeatherLock::SetEnabled(false);
                }
            ...
            case SKSEMenuFramework::Model::kBeforeRender:
                ...
                if (menuFrameworkOpen)
                {
                    WeatherLock::Maintain();
                }
                break;
            case SKSEMenuFramework::Model::kAfterRender:
                if (!menuEffectOverride.lumaRenderedThisFrame)
                {
                    RestoreFrameworkEffects(true);
                }
                break;
            case SKSEMenuFramework::Model::kCloseMenu:
                {
                    ...
                    const auto weatherLockWasEnabled = WeatherLock::IsEnabled();
                    menuFrameworkOpen = false;
                    WeatherLock::SetEnabled(false);
                    WeatherLock::SetSelectedWeather(nullptr);
                    activeWeatherLockProfile.clear();
                    statusMessage.clear();
                    settingsStatusMessage.clear();
                    const auto reconcileEffects = menuEffectOverride.lumaVisitedThisSession;
                    const auto expectedBlurCount = menuEffectOverride.expectedBlurCountAfterClose;
                    ReleaseFrameworkEffectsAfterClose();
                    menuEffectOverride.lumaRenderedThisFrame = false;
                    menuEffectOverride.lumaVisitedThisSession = false;
                    if (reconcileEffects || weatherLockWasEnabled)
                    {
                        ScheduleClosedMenuStateReconciliation(
        ...
        SliderRange FallbackSliderRange(const std::string_view a_setting)
        {
            ...
            if (setting.contains("compression")) return { -200.0f, 200.0f };
            ...
            const SliderInputRange a_inputRange = SliderInputRange::standard)
        {
            ...
            if (ImGuiMCP::SliderFloat(sliderID.c_str(), &sliderValue, a_minimum, a_maximum, ""))
            {
                a_value = SnapSliderValue(sliderValue, a_minimum, a_maximum, a_step);
                changed = true;
            }
            ...
            const auto inputMinimum =
                a_inputRange == SliderInputRange::hue ? 0.0f : a_minimum < 0.0f ? -99.0f :
                                                                                 0.0f;
            const auto inputMaximum = a_inputRange == SliderInputRange::hue ? 255.0f : 99.0f;
        ...
        bool DrawDynamicAmbientRangeSlider(
            const std::string_view a_setting,
            const std::string& a_id,
            float& a_darkLimit,
            float& a_brightLimit)
        {
            ...
            const auto itemID = ImGuiMCP::GetID(a_id.c_str());
            ...
            const float handleHitRadius = handleThickness * 0.75f;
            ImGuiMCP::InvisibleButton(a_id.c_str(), ImGuiMCP::ImVec2(width, height));

            if (ImGuiMCP::IsItemActivated())
            {
                const float mouseX = std::clamp(ImGuiMCP::GetMousePos().x, position.x, position.x + width);
                const float darkX = position.x + (width * a_darkLimit / 255.0f);
                const float brightX = position.x + (width * a_brightLimit / 255.0f);
                const float darkDistance = std::abs(mouseX - darkX);
                const float brightDistance = std::abs(mouseX - brightX);
                const bool handleSelected =
                    darkDistance <= handleHitRadius ||
                    brightDistance <= handleHitRadius;
                if (handleSelected || (mouseX > darkX && mouseX < brightX))
                {
                    dynamicAmbientDragStates[itemID] = {
                        .target = handleSelected ?
                                      (darkDistance <= brightDistance ?
                                              DynamicAmbientDragTarget::darkLimit :
                                              DynamicAmbientDragTarget::brightLimit) :
                                      DynamicAmbientDragTarget::range,
                        .startMouseX = mouseX,
                        .startDarkLimit = a_darkLimit,
                        .startBrightLimit = a_brightLimit,
                    };
                }
                else
                {
                    dynamicAmbientDragStates.erase(itemID);
                }
            }
            ...
            {
                const float mouseX = std::clamp(ImGuiMCP::GetMousePos().x, position.x, position.x + width);
                const auto drag = dynamicAmbientDragStates.find(itemID);
                if (drag != dynamicAmbientDragStates.end())
                {
                    if (drag->second.target == DynamicAmbientDragTarget::range)
                    {
                        const float span = drag->second.startBrightLimit - drag->second.startDarkLimit;
                        const float translated = SnapSliderValue(
                            drag->second.startDarkLimit +
                                (255.0f * (mouseX - drag->second.startMouseX) / width),
                            0.0f,
                            255.0f - span,
                            std::max(1.0f, style.step));
                        const float translatedBright = translated + span;
                        changed =
                            std::abs(translated - a_darkLimit) > 0.0001f ||
                            std::abs(translatedBright - a_brightLimit) > 0.0001f;
                        a_darkLimit = translated;
                        a_brightLimit = translatedBright;
                    }
                    else
                    {
                        const float value = SnapSliderValue(
                            255.0f * (mouseX - position.x) / width,
                            0.0f,
                            255.0f,
                            std::max(1.0f, style.step));
                        if (drag->second.target == DynamicAmbientDragTarget::darkLimit)
                        {
                            const float limited = std::min(value, a_brightLimit);
                            changed = std::abs(limited - a_darkLimit) > 0.0001f;
                            a_darkLimit = limited;
                        }
                        else
                        {
                            const float limited = std::max(value, a_darkLimit);
                            changed = std::abs(limited - a_brightLimit) > 0.0001f;
                            a_brightLimit = limited;
                        }
                    }
                }
            }
            ...
            const float darkX = position.x + (width * a_darkLimit / 255.0f);
            const float brightX = position.x + (width * a_brightLimit / 255.0f);
            const float mouseX = ImGuiMCP::GetMousePos().x;
            const float darkDistance = std::abs(mouseX - darkX);
            const float brightDistance = std::abs(mouseX - brightX);
            const bool darkHovered =
                ImGuiMCP::IsItemHovered() &&
                darkDistance <= handleHitRadius &&
                darkDistance <= brightDistance;
            const bool brightHovered =
                ImGuiMCP::IsItemHovered() &&
                brightDistance <= handleHitRadius &&
                brightDistance < darkDistance;
            const bool rangeHovered =
                ImGuiMCP::IsItemHovered() &&
                !darkHovered &&
                !brightHovered &&
                mouseX > darkX &&
                mouseX < brightX;
            const auto drag = dynamicAmbientDragStates.find(itemID);
            const bool rangeActive =
                ImGuiMCP::IsItemActive() &&
                drag != dynamicAmbientDragStates.end() &&
                drag->second.target == DynamicAmbientDragTarget::range;
            const bool darkActive =
                ImGuiMCP::IsItemActive() &&
                drag != dynamicAmbientDragStates.end() &&
                drag->second.target == DynamicAmbientDragTarget::darkLimit;
            const bool brightActive =
                ImGuiMCP::IsItemActive() &&
                drag != dynamicAmbientDragStates.end() &&
                drag->second.target == DynamicAmbientDragTarget::brightLimit;
            const auto* normalRangeColor = ImGuiMCP::GetStyleColorVec4(ImGuiMCP::ImGuiCol_SliderGrab);
            const auto* activeRangeColor = ImGuiMCP::GetStyleColorVec4(ImGuiMCP::ImGuiCol_SliderGrabActive);
            const auto normalColor = normalRangeColor ?
                                         *normalRangeColor :
                                         ImGuiMCP::ImVec4(0.24f, 0.52f, 0.88f, 1.0f);
            const auto activeColor = activeRangeColor ?
                                         *activeRangeColor :
                                         ImGuiMCP::ImVec4(0.35f, 0.65f, 1.0f, 1.0f);
            const auto rangeColor = ImGuiMCP::GetColorU32(
                rangeActive ? activeColor :
                rangeHovered ? ImGuiMCP::ImLerp(normalColor, activeColor, 0.55f) :
                               normalColor);
            const float hoveredHandleThickness = std::min(height, handleThickness * 1.25f);
            const float darkThickness = darkHovered || darkActive ? hoveredHandleThickness : handleThickness;
            const float brightThickness = brightHovered || brightActive ? hoveredHandleThickness : handleThickness;
            ...
            const std::size_t a_index)
        {
            ...
            auto& configured =
                ambient ?
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            settings.dynamicAmbientWithin :
                            settings.dynamicAmbientBetween) :
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            settings.dynamicSunlightWithin :
                            settings.dynamicSunlightBetween);
            const auto stateKey =
                profile +
            ...
            const auto labelKey =
                ambient ?
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            "dynamicAmbientWithin" :
                            "dynamicAmbientBetween") :
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            "dynamicSunlightWithin" :
                            "dynamicSunlightBetween");
            const auto fallbackLabel =
                ambient ?
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            "Dynamic Ambient Within" :
                            "Dynamic Ambient Between") :
                    (a_mode == WeatherPatcher::DynamicAmbientMode::within ?
                            "Dynamic Sunlight Within" :
                            "Dynamic Sunlight Between");
            ImGuiMCP::TextUnformatted(SKSEMenuSettings::Label(labelKey, fallbackLabel).c_str());
            if (!state.range.available)
            {
                ImGuiMCP::BeginDisabled();
            }

            float darkLimit = static_cast<float>(configured.darkLimit.value_or(state.range.darkLimit));
            float brightLimit = static_cast<float>(configured.brightLimit.value_or(state.range.brightLimit));
            darkLimit = std::clamp(darkLimit, 0.0f, 255.0f);
            brightLimit = std::clamp(brightLimit, darkLimit, 255.0f);
            const auto sliderID = std::format(
                "##{}{}{}DynamicBrightness{}",
                profile,
                ambient ? "Ambient" : "Sunlight",
                a_mode == WeatherPatcher::DynamicAmbientMode::within ? "Within" : "Between",
                a_index);
            if (DrawDynamicAmbientRangeSlider(labelKey, sliderID, darkLimit, brightLimit))
            {
            ...
            if (state.status && state.status->compression)
            {
                ImGuiMCP::Text(
                    "%s: %.0f%%",
                    SKSEMenuSettings::Label("dynamicCompression", "Compression").c_str(),
                    *state.status->compression);
            }
            else
            {
                ImGuiMCP::Text(
                    "%s: %s",
                    SKSEMenuSettings::Label("dynamicCompression", "Compression").c_str(),
                    SKSEMenuSettings::Label("notAvailable", "N/A").c_str());
            }
            ImGuiMCP::Text(
                "%s: %.0f    %s: %.0f",
                SKSEMenuSettings::Label("darkBrightness", "Dark Brightness").c_str(),
                result.darkLimit,
                SKSEMenuSettings::Label("brightBrightness", "Bright Brightness").c_str(),
                result.brightLimit);
            if (!state.range.available)
            {
        ...
        void ReloadProfileMenus(const bool a_initialLoad)
        {
            ...
            const auto filteredRulesChanged = TuningUtil::ReloadFilteredWeatherRules();
        ...
        const std::vector<WeatherMenuEntry>& GetSliderCreatorWeatherEntries(const std::string& a_profileName)
        {
        ...
        std::string GetCurrentRegion()
        {
            ...
            if (!stat->mmsfAPI) stat->mmsfAPI = API::RequestMMSFAPI();
        ...
        void SelectWeather(const std::string& a_profile, RE::TESWeather* a_weather)
        {
            ...
            const auto lockEnabled =
                settings.EnableProfile && weatherLockPreferences[profile] && menuFrameworkOpen;
            WeatherLock::SetSelectedWeather(a_weather);
            WeatherLock::SetEnabled(lockEnabled);
            WeatherSyncClient::SetWeatherInstant(
                a_weather,
                lockEnabled);
            statusMessage.clear();
        }
        ...
        void DrawTimeOfDayControls(const std::string& a_id);

        void DrawWeatherControlCompact(const MenuDefinition& a_menu, const std::string& a_id)
        {
            DrawTimeOfDayControls(a_id + "Time");
            DrawQuickSelect(a_menu, a_id + "QuickSelect", false);
            DrawWeatherLockToggle(a_menu, "##LockWeather" + a_id);
            ImGuiMCP::SameLine();
            ImGuiMCP::TextUnformatted("Lock Weather");
        }
            ...
            const std::string& a_id)
        {
            ...
            const auto enable = menuFrameworkOpen && settings.EnableProfile && lockEnabled && selectedWeather;
            WeatherLock::SetEnabled(enable);
            if (enable)
            {
                WeatherSyncClient::SetWeatherInstant(
        ...
        void DrawTimeOfDayControls(const std::string& a_id)
        {
            ...
            const auto* format =
                IsSafeSliderFormat(style.format) ?
                    style.format.c_str() :
                    fallbackFormat;
            constexpr auto timeLabel = "Time of Day";

            ImGuiMCP::TextUnformatted(timeLabel);
            ImGuiMCP::SameLine();
            ImGuiMCP::SetNextItemWidth(inputWidth);
            ...
            if (ImGuiMCP::InputFloat(
                    ...
                    format))
            {
                value = std::clamp(inputValue, 0.0f, 24.0f);
                changed = true;
            }

            auto sliderValue = std::clamp(value, 0.0f, 24.0f);
            if (style.width != 0.0f)
            {
                ImGuiMCP::SetNextItemWidth(style.width);
            }
            ...
            if (changed)
            {
                calendar->gameHour->value = std::clamp(value, 0.0f, 24.0f);
                WeatherLock::Maintain(true);
            }
        }
        ...
        bool DrawWeatherLinks(TuningUtil::Settings& a_settings, const std::string& a_prefix)
        {
        ...
        bool DrawInteriorLinks(TuningUtil::Settings& a_settings, const std::string& a_prefix)
        {
            return DrawLinkOnlyCategory(
                "Interior Links",
        ...
        bool DrawRecordFilterEditor(
            ...
            const std::string_view a_id)
        {
            const auto originalInclusions = a_inclusions;
            const auto originalExclusions = a_exclusions;
            ...
            if (ImGuiMCP::Button((SKSEMenuSettings::Label("addToIncluded", "Add to Included") +
                        ...
                        .c_str()))
            {
                AddUniqueString(a_inclusions.formIDs, recordKey);
                std::erase_if(a_exclusions.formIDs, [&](const auto& a_value)
            ...
            if (ImGuiMCP::Button((SKSEMenuSettings::Label("addToExcluded", "Add to Excluded") +
                        ...
                        .c_str()))
            {
                AddUniqueString(a_exclusions.formIDs, recordKey);
                std::erase_if(a_inclusions.formIDs, [&](const auto& a_value)
                    { return Config::IEquals(a_value, recordKey); });
            }
            ImGuiMCP::EndDisabled();
            ...
            return originalInclusions != a_inclusions ||
                   originalExclusions != a_exclusions ||
            ...
            const std::size_t a_index)
        {
            ...
            else
            {
                ...
                {
                    ApplySliderChange(
                        pointLightHueScalesChanged || hueRangesChanged);
                }
                return;
            }
        ...
        void DrawLinksModule(const MenuDefinition& a_menu, const MenuControl& a_control)
        {
            ...
            if (Config::IEquals(a_control.setting, "weather"))
                changed |= DrawWeatherLinks(settings, profile + "LinksWeather");
            else if (Config::IEquals(a_control.setting, "interior"))
                changed |= DrawInteriorLinks(settings, profile + "LinksInterior");
            ...
            const std::string& a_idPrefix)
        {
            ...
            constexpr std::array<std::pair<std::string_view, std::string_view>, 5> fields{
                std::pair{ "saturation", "Saturation" },
                std::pair{ "brightness", "Brightness" },
                std::pair{ "contrast", "Contrast" },
                std::pair{ "sunlightScale", "Sunlight Scale" },
                std::pair{ "skyScale", "Sky Scale" },
            };
            ...
            const MenuControl& a_control)
        {
            ...
            if (a_category == "brightness")
            {
                ...
                {
                    changed |= DrawLinkableCategory(fields, 0.1f, 4.0f, prefix, 0.1f, 1.0);
            ...
            else if (a_category == "saturation")
            {
                ...
                {
                    changed |= DrawLinkableCategory(fields, 0.0f, 6.0f, prefix, 0.1f, 0.0, 6.0f);
                }
            }
            else if (a_category == "hueScales")
            {
                auto fields = HueScaleFields(settings.hueScales);
                changed |= DrawValueOnlyCategory(fields, 0.0f, 4.0f, prefix, 0.1f);
            }
            ...
            else if (a_category == "betweenCompression")
            {
                ...
                changed |= DrawLinkableCategory(betweenFields, -200.0f, 200.0f, prefix + "between", 10.0f);
            }
            else if (a_category == "withinCompression")
            {
                ...
                changed |= DrawLinkableCategory(withinFields, -200.0f, 200.0f, prefix + "within", 10.0f);
            }
            else if (a_category == "exteriorImageSpace")
            {
                changed |= DrawImageSpaceSettings(settings.exteriorImageSpace, prefix);
            }
            ...
            const std::string_view a_category)
        {
            ...
            if (a_category == "intBrightness")
            {
                ...
                changed |= DrawLinkableCategory(fields, 0.0f, 10.0f, prefix, 0.1f);
            ...
            else if (a_category == "pointLights")
            {
                ...
                drawMultiplier("Fade", "fade", settings.pointLights.fadeMultiplier, 10.0f);
                drawMultiplier("Saturation", "saturation", settings.pointLights.saturationMultiplier, 6.0f);

                DrawHeader("Hue Scales");
                auto hueScales = HueScaleFields(settings.pointLights.hueScales);
                changed |= DrawValueOnlyCategory(hueScales, 0.0f, 4.0f, prefix + "hueScales", 0.1f);
            ...
            {
                changed |= DrawImageSpaceSettings(settings.intImageSpace, prefix);
            }
            else supported = false;
                        ...
                        if (selected)
                        {
                            ...
                            if (WeatherPatcher::PreviewPreset(profile, category, preset, error))
                            {
                                ...
                                statusMessage = active ?
                                                    StatusText("presetDeselected", { { "category", category } }) :
                                                    StatusText(
                                                        "presetPreview",
                                                        {
                                                            { "category", category },
        ...
        void DrawPresetCreator(const MenuDefinition& a_menu, const MenuControl& a_control)
        {
            ...
            const auto heading = a_control.header.empty() ?
                                     SKSEMenuSettings::Label("presetCreatorHeader", "Create Presets") :
                                     a_control.header;
            ...
            if (ImGuiMCP::BeginChild(savePanelId.c_str(), ImGuiMCP::ImVec2(0.0f, 0.0f), savePanelFlags))
            {
                ...
                DrawHeader(SKSEMenuSettings::Label("removePresetOrCategory", "Remove Preset or Category"));
                ...
                if (ImGuiMCP::BeginCombo(("Category##" + profile + "PresetRemovalCategory").c_str(), categoryPreview.c_str()))
                {
                ...
                if (ImGuiMCP::BeginCombo(("Preset##" + profile + "PresetRemovalName").c_str(), presetPreview.c_str()))
                {
                    for (const auto& preset : presets)
                    {
                ...
                if (ImGuiMCP::Button((removePresetLabel + "##" + profile).c_str()))
                {
                ...
                if (ImGuiMCP::Button((removeCategoryLabel + "##" + profile).c_str()))
                {
                    const auto category = input.removalCategory;
        ...
        {
            DrawSavePresetSelection(a_menu, a_control, a_menu.profile + "PresetCommit");
            ImGuiMCP::Spacing();
            ...
            const SliderCreator::ExistingSlider& a_slider)
        {
            ...
            auto domain = definition.filtered ?
            ...
            const RecordFilterKind a_kind)
        {
            if (ImGuiMCP::BeginListBox(a_id.c_str(), ImGuiMCP::ImVec2(0.0f, 150.0f)))
            {
                ...
                for (std::size_t index = 0; index < a_values.size();)
                {
                    ...
                    auto* record = a_kind == RecordFilterKind::lightingTemplate ?
                                       static_cast<RE::TESForm*>(form.Get<RE::BGSLightingTemplate>()) :
                                       static_cast<RE::TESForm*>(form.Get<RE::TESWeather>());
        ...
        void DrawSliderCreatorSettings(SliderCreatorState& a_state, const std::string& a_id)
        {
            ...
            const auto filteredTarget = a_state.domain == SliderCreatorDomain::weather &&
                                        a_state.filtered &&
        ...
        SliderCreator::Definition CreatorDefinition(const SliderCreatorState& a_state)
        {
            ...
            const auto filtered = a_state.domain == SliderCreatorDomain::weather && a_state.filtered;
            result.link = filtered ? "" : InputText(a_state.link);
            result.localLink = filtered ? InputText(a_state.localLink) : "";
            ...
            result.useTimes = filtered && a_state.useTimes;
        ...
        std::string SliderCreatorStyleSetting(const SliderCreatorState& a_state)
        {
            ...
            if (a_state.domain != SliderCreatorDomain::weather || !a_state.filtered) return setting;
        ...
        {
            const auto filteredFeatures = a_state.domain == SliderCreatorDomain::weather &&
                                          a_state.filtered &&
                                          a_operation.has_value();
            const auto supportsHueScales = filteredFeatures &&
            ...
            const auto formatHint = std::format(
                "{}: {}",
            ...
            const SliderCreatorDomain a_domain)
        {
            ...
            const auto supportsFiltering =
                state.domain == SliderCreatorDomain::weather &&
                (state.settings.empty() ||
                 filteredOperation.has_value());
            if (!supportsFiltering) state.filtered = false;
            if (supportsFiltering)
                ImGuiMCP::Checkbox(("Filtered Weather Slider##" + stateKey).c_str(), &state.filtered);
            ...
            for (const auto& control : a_controls)
            {
                ...
                else if (control.kind == "interiorSetup")
                {
                    static constexpr std::array scopes{
                        ...
                        "effectLightingInclusions",
                        "effectLightingExclusions",
            ...
            const std::size_t a_index)
        {
            ...
            const auto drawText = [&](const bool a_separator)
            {
                ...
                if (a_separator)
                {
                    DrawHeader(a_module.label, scale);
                    return;
                }
                if (scale != 1.0f) ImGuiMCP::SetWindowFontScale(scale);
                ImGuiMCP::TextWrapped("%s", a_module.label.c_str());
                if (scale != 1.0f) ImGuiMCP::SetWindowFontScale(1.0f);
            };
            ...
            if (a_module.kind == "slider")
            {
                const auto fallbackLabel = !a_module.settings.empty() ?
                                               ...
                                               a_module.setting;
                const auto visibleLabel = a_module.label.empty() ? fallbackLabel : a_module.label;
                const auto label = visibleLabel + "##" + a_menu.profile + "MenuModule" + std::to_string(a_index);
                const auto& profile = a_menu.profile;
                const auto filtered = !a_module.id.empty() && TuningUtil::FindFilteredWeatherRule(profile, a_module.id);
                const auto drawn = filtered ?
        ...
        void DrawItemTooltip(const std::string& a_tooltip)
        {
            ...
            if (!a_tooltip.empty() && ImGuiMCP::IsItemHovered(hoverFlags))
            {
                ...
                if (ImGuiMCP::BeginTooltip())
                {
                    ...
                    if (scale != 1.0f) ImGuiMCP::SetWindowFontScale(scale);
                    ImGuiMCP::TextWrapped("%s", a_tooltip.c_str());
                    if (scale != 1.0f) ImGuiMCP::SetWindowFontScale(1.0f);
                    ImGuiMCP::EndTooltip();
                }
            ...
            const std::string_view a_pageID)
        {
            ...
            for (std::size_t index = 0; index < a_modules.size(); ++index)
            {
                ...
                if (module.kind == "boxStart")
                {
                    ...
                    if (visible && !module.label.empty()) DrawHeader(module.label);
        ...
        constexpr std::array kLayoutModuleChoices{
            ...
            LayoutModuleChoice{ "Presets Create", "presetCreator", "", "" },
            ...
            LayoutModuleChoice{ "Weather Hue Scales", "settings", "hueScales", "" },
            ...
            LayoutModuleChoice{ "Dynamic Ambient Between", "dynamicAmbientBetween", "", "" },
            LayoutModuleChoice{ "Dynamic Ambient Within", "dynamicAmbientWithin", "", "" },
            ...
            LayoutModuleChoice{ "Interior Hue Scales", "settings", "intHueScales", "" },
        ...
        void DrawProfilePageEditor(const LoadedMenu& a_menu)
        {
            ...
            if (ImGuiMCP::BeginTable(tableID.c_str(), 4, tableFlags))
            {
