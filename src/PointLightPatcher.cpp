...
namespace MPL::PointLightPatcher
{
    namespace
    {
        ...
        struct ReferenceRuntimeFadeBaseline
        {
            float sourceFade;
            float appliedFade;
            bool hasApplied = false;
        ...
        std::unordered_map<RE::TESObjectREFR*, ReferenceRuntimeFadeBaseline>
        ...
        LoadedReferenceRefreshResult RefreshLoadedLightReferences(
            const AppliedState&,
            bool = true,
            std::string_view = "loaded-reference-refresh");
        ...
        double OriginalBaseRadius(const RE::TESObjectLIGH* a_light)
        {
            if (!a_light) return 0.0;
            const auto baseline = baselines.find(const_cast<RE::TESObjectLIGH*>(a_light));
            return baseline != baselines.end() ? baseline->second.radius : a_light->data.radius;
        }

        void UpdateRuntimeRadius(RE::NiLight* a_light, const float a_radius)
        {
            auto* pointLight = netimmerse_cast<RE::NiPointLight*>(a_light);
            if (!pointLight) return;
            auto& runtime = pointLight->GetLightRuntimeData();
            if (runtime.radius.x == a_radius && runtime.radius.y == a_radius && runtime.radius.z == a_radius) return;
            runtime.radius = RE::NiPoint3{ a_radius, a_radius, a_radius };
            // The engine takes an integer radius, not the float declared by CommonLib.
            using SetAttenuation = void (*)(RE::NiPointLight*, std::uint32_t);
            static REL::Relocation<SetAttenuation> setAttenuation{ RELOCATION_ID(17224, 17626) };
            setAttenuation(pointLight, static_cast<std::uint32_t>(a_radius));
            RE::NiUpdateData update{};
            update.flags.set(RE::NiUpdateData::Flag::kDirty);
            pointLight->Update(update);
        }

        void RefreshReferenceRadius(RE::TESObjectREFR* a_reference, const AppliedState& a_state)
        {
            const auto* light = GetBaseLight(a_reference);
            if (!light || !a_reference->Is3DLoaded()) return;
            const auto multiplier = PointLightFade::ResolveRadius(
                SettingsForBaseLight(light, a_state).radiusMultiplier,
                light->GetFormID(), a_state.referenceRules, [&](const auto& filter)
                { return RecordFilter::Matches(ExternalEmittanceSource(a_reference), filter); });
            const auto* extraRadius = a_reference->extraList.GetByType<RE::ExtraRadius>();
            const auto radius = static_cast<float>(PointLightRadius::Scale(
                OriginalBaseRadius(light), extraRadius ? extraRadius->radius : 0.0, multiplier));
            if (auto* loaded = a_reference->loadedData)
                loaded->cachedRadius = radius;
            const auto* extraLight = a_reference->extraList.GetByType<RE::ExtraLight>();
            if (extraLight && extraLight->lightData)
                UpdateRuntimeRadius(extraLight->lightData->light.get(), radius);
        }
        ...
        EmittanceMap LoadEmittanceMappings()
        {
            ...
            for (const auto& path : files)
            {
                ...
                if (!text)
                {
                    logger::warn("[Point Lights] XEMIUtil mapping read failed | file={}", path.string());
                    continue;
                }
                ...
                {
                    logger::warn("[Point Lights] XEMIUtil mapping parse failed | file={}", path.string());
                    continue;
                }
            ...
            DetailedLogging::Info("[Point Lights] XEMIUtil mappings={}", result.size());
            return result;
        }
        ...
        yyjson_mut_val* CopyValue(
            ...
            const double a_radiusMultiplier,
            ...
            const ColorTuning& a_tuning)
        {
            ...
            while (auto* key = yyjson_obj_iter_next(&iterator))
            {
                ...
                else if (KeyEquals(key, "radiusController"))
                {
                    copiedValue = CopyFadeControllerValue(a_document, value, a_radiusMultiplier);
                }
                ...
                {
                    copiedValue = CopyValue(a_document, value, a_fadeMultiplier, a_radiusMultiplier, a_tuning);
                }
                if (!copiedKey || !copiedValue || !yyjson_mut_obj_add(result, copiedKey, copiedValue))
                {
                    return nullptr;
            ...
            const ColorTuning& a_tuning)
        {
            if (yyjson_is_obj(a_value))
            {
                return CopyObject(a_document, a_value, a_fadeMultiplier, a_radiusMultiplier, a_tuning);
            }
            if (yyjson_is_arr(a_value))
            {
                auto* result = yyjson_mut_arr(a_document);
                ...
                yyjson_arr_foreach(a_value, index, count, entry)
                {
                    auto* copy = CopyValue(a_document, entry, a_fadeMultiplier, a_radiusMultiplier, a_tuning);
                    if (!copy || !yyjson_mut_arr_append(result, copy))
                    {
                        return nullptr;
                    }
            ...
            const EmittanceMap& a_mappings)
        {
            ...
            const auto radius = PointLightFade::ResolveRadius(
                settings.radiusMultiplier, baseLight ? baseLight->GetFormID() : 0,
            ...
            auto* result = CopyObject(a_document, a_data, fade, radius, tuning);
            if (result && (radius != 1.0 || settings.radiusMultiplier != 1.0))
            {
                auto* configuredRadius = yyjson_obj_get(a_data, "radius");
                const auto sourceRadius = yyjson_is_num(configuredRadius) ? yyjson_get_num(configuredRadius) : 0.0;
                yyjson_mut_obj_remove_key(result, "radius");
                if (!yyjson_mut_obj_add_real(a_document, result, "radius",
                        PointLightRadius::LightPlacer(sourceRadius, OriginalBaseRadius(baseLight), radius)))
                    return nullptr;
            }
            ...
            void* a_context)
        {
            ...
            {
                logger::error(
                    "[Point Lights] Light Placer transform failed | {}",
                    error.what());
            }
            catch (...)
            {
                logger::error("[Point Lights] Light Placer transform failed | unknown exception");
            }
            return false;
        }
        ...
        void OnBrokeredLightPlacerReloadComplete(const bool a_succeeded)
        {
            ...
            if (!a_succeeded)
            {
                ...
                logger::warn("[Point Lights] Light Placer reload failed | broker=Heliosphan | retry=true");
            }
        ...
        bool ConnectLightPlacerBroker()
        {
                ...
                !heliosphanAPI->RegisterReferenceClient)
            {
                ...
                DetailedLogging::Info("{}", "[Point Lights] Light Placer broker=standalone | Heliosphan unavailable");
                return false;
            }
            ...
            {
                heliosphanAPI = nullptr;
                logger::warn("[Point Lights] Light Placer broker=standalone | transformer rejected");
                return false;
            }
            ...
            {
                logger::warn("[Point Lights] Heliosphan reference client=rejected");
            }
            logger::info(
                "[Point Lights] Heliosphan | LightPlacer=true | referenceNotifications={}",
                heliosphanReferenceNotifications);
            return true;
        }
        ...
        bool ExecuteLightPlacerReload()
        {
            ...
            if (!command)
            {
                DetailedLogging::Info("{}", "[Point Lights] Light Placer reload=skipped | command unavailable");
                return false;
            }
            auto* script = RE::IFormFactory::Create<RE::Script>();
            if (!script)
            {
                logger::error("[Point Lights] Light Placer reload failed | console script unavailable");
                return false;
            }
        ...
        void ReloadLightPlacer(const AppliedState& a_state)
        {
                ...
                iterator.increment(error))
            {
                ...
                if (!original)
                {
                    logger::warn("[Point Lights] Light Placer read failed | file={}", iterator->path().string());
                    continue;
                }
                const auto transformed = TransformLightPlacerJson(*original, a_state, mappings);
                if (!transformed)
                {
                    logger::warn("[Point Lights] Light Placer transform failed | file={} | unchanged=true", iterator->path().string());
                    continue;
                }
            ...
            for (; attempted < replacements.size(); ++attempted)
            {
                ...
                {
                    logger::error("[Point Lights] Light Placer temporary write failed | file={}", replacements[attempted].path.string());
                    ++attempted;
            ...
            for (std::size_t i = 0; i < attempted; ++i)
            {
                ...
                {
                    logger::critical("[Point Lights] Light Placer restore failed | file={}", replacements[i].path.string());
                }
            }
            if (writeSucceeded)
            {
                logger::info("[Point Lights] Light Placer reload | files={} | restored=true", attempted);
            }
            ...
            const AppliedState& a_state)
        {
            ...
            if (settings.fadeMultiplier != previous.fadeMultiplier ||
                settings.radiusMultiplier != previous.radiusMultiplier ||
            ...
            if (std::ranges::any_of(candidates, [&](const RE::FormID a_formID)
                {
                    const auto& currentSettings = SettingsForBaseLight(a_formID, a_state);
                    const auto& previousSettings = SettingsForBaseLight(a_formID, *a_previous);
                    return currentSettings.fadeMultiplier != previousSettings.fadeMultiplier ||
                        currentSettings.radiusMultiplier != previousSettings.radiusMultiplier;
                }))
                return true;
            if constexpr (kUseDirectLightPlacerNiLights)
            {
            ...
            DirectLightRefreshResult* a_result = nullptr)
        {
            ...
            else if (a_logEmptyResult || lightCount != 0)
            {
                DetailedLogging::Info(
                    "[Point Lights] Light Placer runtime | changed={}/{} | references={}",
                    changedCount,
                    lightCount,
                    loadedReferenceCount);
            }
            ...
            std::thread(
                [a_generation, a_attempt, a_retryUntilFound]()
                {
                            ...
                            a_retryUntilFound]()
                    {
                        ...
                        {
                            DetailedLogging::Info(
                                "[Point Lights] Light Placer post-reload | nodes={} | attempt={}/{}",
                                lightCount,
                                a_attempt,
                        ...
                        {
                            logger::warn(
                                "[Point Lights] Light Placer post-reload | nodes=0 | attempts={} | cellRefresh=true",
                                kPostReloadRefreshAttempts);
                        }
                        else
                        {
                            DetailedLogging::Info("{}", "[Point Lights] Light Placer post-reload | nodes=0 | retries=false");
                        }
                    };
        ...
        void QueueLoadedPointLightRefresh(const RE::FormID a_cell)
        {
            ...
            else
            {
                loadedReferenceRefreshPending.store(
                    false,
                    std::memory_order_release);
                logger::warn("[Point Lights] loaded-cell refresh queue failed | game thread unavailable");
            }
        }
        ...
        class EquippedLightEventSink final :
        ...
        public:
                ...
                RE::BSTEventSource<RE::TESEquipEvent>*) override
            {
                ...
                {
                    if (auto* light = RE::TESForm::LookupByID<RE::TESObjectLIGH>(a_event->baseObject);
                        light && light->CanBeCarried())
                        QueueReferenceReconciliation(a_event->actor.get());
                }
                return RE::BSEventNotifyControl::kContinue;
            }
                ...
                RE::BSTEventSource<RE::TESObjectLoadedEvent>*) override
            {
                if (a_event && a_event->loaded)
                {
                    if (auto* actor = RE::TESForm::LookupByID<RE::Actor>(a_event->formID))
                        QueueReferenceReconciliation(actor);
                }
                return RE::BSEventNotifyControl::kContinue;
            }
                ...
                RE::BSTEventSource<RE::TESCellAttachDetachEvent>*) override
            {
                if (a_event && a_event->attached && a_event->reference &&
                    a_event->reference->Is(RE::FormType::ActorCharacter))
                    QueueReferenceReconciliation(a_event->reference.get());
                return RE::BSEventNotifyControl::kContinue;
            }
        };

        EquippedLightEventSink equippedLightEventSink;
                ...
                "reference-fade")
        {
            ...
            if (baseline == referenceFadeBaselines.end())
            {
                baseline = referenceFadeBaselines
                               .emplace(
                                   a_reference,
                                   extraLightData->data.fade)
                               .first;
            ...
            const std::string_view a_context)
        {
            ...
            const bool sharedXEMIBase =
                externallyEmissiveLights.contains(light);
            const auto baseline = baselines.find(light);
            const auto sourceFade = baseline != baselines.end() ?
                                        baseline->second.fade :
                                        light->fade;
            const auto adjustedRecordFade = light->fade;
            const bool correctBaseFadeContribution =
                sharedXEMIBase && baseline != baselines.end();
                ...
                extraLight && extraLight->lightData && extraLight->lightData->light)
            {
                ...
                const bool hasReferenceFade =
                    extraLightData &&
                    (extraLightData->data.fade > 0.0f ||
                     referenceFadeBaselines.contains(a_reference));
                const auto runtimeFadeAfterUpdate = runtime.fade;
                const auto recordFadeAdjustment =
                    correctBaseFadeContribution ?
                        adjustedRecordFade - sourceFade :
                        0.0f;
                const auto normalizedRuntimeFade =
                    std::max(
                        0.0f,
                        runtimeFadeAfterUpdate - recordFadeAdjustment);
                const auto multiplier =
                    std::max(
                        0.0,
                        ReferenceFadeMultiplier(a_reference, a_state));
                float effectiveRuntimeBaseline = sourceFade;
                if (sharedXEMIBase && !hasReferenceFade)
                {
                    auto [runtimeBaseline, inserted] =
                        referenceRuntimeFadeBaselines.try_emplace(
                            a_reference,
                            ReferenceRuntimeFadeBaseline{
                                .sourceFade = normalizedRuntimeFade,
                                .appliedFade = 0.0f,
                                .hasApplied = false,
                            });
                    auto& captured = runtimeBaseline->second;
                    if (!inserted &&
                        (!captured.hasApplied ||
                         !NearlyEqual(
                             runtimeFadeAfterUpdate,
                             captured.appliedFade)))
                    {
                        captured.sourceFade = normalizedRuntimeFade;
                    }
                    effectiveRuntimeBaseline = captured.sourceFade;
                    runtime.fade = effectiveRuntimeBaseline *
                                   static_cast<float>(multiplier);
                    captured.appliedFade = runtime.fade;
                    captured.hasApplied = true;
                }
                else
                {
                    referenceRuntimeFadeBaselines.erase(a_reference);
        ...
        const bool a_commitLightPlacer)
    {
        ...
        if (recordsChanged)
        {
            ...
            for (auto* light : dataHandler->GetFormArray<RE::TESObjectLIGH>())
            {
                ...
                light->data.color = baseline->second.color;
                light->data.radius = PointLightRadius::Record(baseline->second.radius, settings.radiusMultiplier);
            ...
            RefreshDirectLightPlacerLights();
            logger::info(
                "[Point Lights] records | fade={} | color={} | perReference={}",
                fadeCount,
                lightCount,
                externalEmittanceCount);
        }
        ...
        if (a_commitLightPlacer)
        {
            if (RequiresLightPlacerReload(lightPlacerState, state))
            {
                ...
                if (brokered)
                {
                    logger::info("[Point Lights] Light Placer reload | broker=Heliosphan | status=requested");
                }
                else
                {
    ...
    void InstallRuntimeEvents()
    {
        ...
        if (!kUseDirectLightPlacerNiLights)
        {
            logger::info("[Point Lights] Light Placer | mode=JSON reload");
        ...
        if (!eventSource)
        {
            logger::warn("[Point Lights] Light Placer cell refresh=unregistered");
            return;
        }
        eventSource->AddEventSink(
            std::addressof(cellFullyLoadedEventSink));
        eventSource->AddEventSink<RE::TESEquipEvent>(std::addressof(equippedLightEventSink));
        eventSource->AddEventSink<RE::TESObjectLoadedEvent>(std::addressof(equippedLightEventSink));
        eventSource->AddEventSink<RE::TESCellAttachDetachEvent>(std::addressof(equippedLightEventSink));
        runtimeEventsInstalled = true;
        logger::info("[Point Lights] Light Placer | mode=NiLight color/JSON fade and radius");
        logger::info("[Point Lights] equipped lights | equip/load/attach events=registered");
    }
    ...
    void QueueReferenceReconciliation(RE::TESObjectREFR* a_reference)
    {
        if (!a_reference || !a_reference->GetFormID() ||
            (!GetBaseLight(a_reference) && !a_reference->Is(RE::FormType::ActorCharacter)))
        {
            return;
        }
